...
if __name__ == "__main__":
  ...
  try:
    label = str(subprocess.check_output(['git', 'describe', '--tags', '--dirty'], stderr=subprocess.DEVNULL).strip())
  except subprocess.CalledProcessError:
    label = None

  try:
    githash = str(subprocess.check_output(['git', 'rev-parse', '--short', 'HEAD'], stderr=subprocess.DEVNULL).strip().decode())
  except subprocess.CalledProcessError:
    exit(-1)

  try:
    gitbranch = str(subprocess.check_output(['git', 'rev-parse', '--abbrev-ref', 'HEAD'], stderr=subprocess.DEVNULL).strip().decode())
  except subprocess.CalledProcessError:
    exit(-1)
